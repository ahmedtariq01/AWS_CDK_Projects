...
# resource in crud_operation_web_app_health_check/crud_operation_web_app_health_check_stack.py
@pytest.fixture
def app():
    app = core.App()
...
def test_sns(app):
    app.resource_count_is("AWS::SNS::Topic", 1)
    
# check the lambda handler function name
def test_lambda_handler(app):
    app.has_resource_properties("AWS::Lambda::Function", {
...
def test_lambda_policy(app):
    app.resource_count_is("AWS::IAM::Policy", 1)
    
# check the cloudwatch metrics
def test_cloudwatch_metrics(app):
    app.has_resource_properties("AWS::CloudWatch::Alarm", {
